...
class Particle:
    def __init__(self, x, y, color):
        ...
        if isinstance(color, str):
            self.color = COLORS.get(color, (random.randint(100, 255), random.randint(100, 255), random.randint(100, 255)))
        else:
            self.color = color[:3]
...
class Menu:
    def __init__(self, screen_width, screen_height):
        ...
        self.buttons = [
            {"text": "Iniciar Jogo", "action": "start", "rect": pygame.Rect(0, 0, 300, 50)},
            {"text": "Sair", "action": "quit", "rect": pygame.Rect(0, 0, 300, 50)}
        ]
    ...
    def initialize_particles(self):
        ...
        for _ in range(30):
            x = random.randint(self.screen_width//2 - 200, self.screen_width//2 + 200)
    ...
    def draw(self, screen):
        ...
        for p in self.background_particles:
            shape_surf = pygame.Surface((p["size"], p["size"]), pygame.SRCALPHA)
        ...
        title_rect = title_text.get_rect(center=(self.screen_width//2, self.title_pos_y))
        
        # Efeito de brilho no título
        glow = pygame.Surface((title_rect.width + 20, title_rect.height + 20), pygame.SRCALPHA)
        ...
        for i in range(3):
            pygame.draw.rect(glow, (*COLORS["white"], 10 + i*10), 
                            (i*5, i*5, glow.get_width()-i*10, glow.get_height()-i*10), 
                            border_radius=10)
        ...
        for p in self.title_particles:
            ...
            shape_surf = pygame.Surface((block_size * len(shape[0]), block_size * len(shape)), pygame.SRCALPHA)
        ...
        for i, button in enumerate(self.buttons):
            ...
            pygame.draw.rect(self.surface, color, button["rect"], border_radius=10)
            pygame.draw.rect(self.surface, COLORS["white"], button["rect"], 2, border_radius=10)
            
            text = self.button_font.render(button["text"], True, COLORS["white"])
            text_rect = text.get_rect(center=button["rect"].center)
            self.surface.blit(text, text_rect)
        
        # Adicionar instruções de controle
        controls_text = FONT.render("Use SETAS ↑↓ e ENTER para selecionar", True, COLORS["white"])
...
class TetrisGame:
        ...
        while self.running:
            ...
            if self.running:
                self.current_song_index = (
                    self.current_song_index + 1) % len(self.playlist)
    ...
    def draw_block(self, surface, x, y, color, size=BLOCK_SIZE, is_current=False):
        """Desenha um bloco com efeitos visuais"""
        rect = pygame.Rect(x, y, size, size)
        
        # Gradiente para o bloco
        if color in COLORS and color != '':
            ...
            shadow = max(0, base_color[0] - 40), max(0, base_color[1] - 40), max(0, base_color[2] - 40)
        ...
        if lines_cleared:
            for y in lines_cleared:
                for x in range(BOARD_WIDTH):
                    if self.board[y][x]:  
                        color = COLORS.get(self.board[y][x], (random.randint(100, 255), random.randint(100, 255), random.randint(100, 255)))
    ...
    def gravity_thread(self):
        while self.running:
            ...
            speed = max(0.05, 0.5 - (self.level * 0.05))
            time.sleep(speed)
    ...
    def draw_sidebar(self):
        # Fundo da sidebar
        sidebar_rect = pygame.Rect(0, PANEL_HEIGHT, SIDEBAR_WIDTH, BOARD_HEIGHT * BLOCK_SIZE)
        ...
        self.game_surface.blit(hold_text, (SIDEBAR_WIDTH//2 - hold_text.get_width()//2, PANEL_HEIGHT + 10))
        
        if self.hold_piece:
            self.draw_piece_preview(self.hold_piece, SIDEBAR_WIDTH//2 - BLOCK_SIZE, PANEL_HEIGHT + 50)
        
        # Next pieces
        next_text = LARGE_FONT.render("NEXT", True, COLORS['white'])
        self.game_surface.blit(next_text, (GAME_WIDTH - SIDEBAR_WIDTH//2 - next_text.get_width()//2, PANEL_HEIGHT + 10))
        
        for i, piece in enumerate(self.next_pieces[:3]):
            self.draw_piece_preview(piece, GAME_WIDTH - SIDEBAR_WIDTH//2 - BLOCK_SIZE, PANEL_HEIGHT + 50 + i * 100)
    ...
    def draw_panel(self):
        ...
        for i in range(PANEL_HEIGHT):
            ...
            pygame.draw.rect(self.game_surface, (138, 43, 226, alpha), line_rect)
        
        # Bordas
        pygame.draw.rect(self.game_surface, COLORS['violeta'], panel_rect, 3)
        pygame.draw.line(self.game_surface, (200, 200, 200), (0, PANEL_HEIGHT-1), (GAME_WIDTH, PANEL_HEIGHT-1), 2)
        
        # Informações
        time_text = FONT.render(f"Tempo: {self.elapsed_time//60:02d}:{self.elapsed_time % 60:02d}", True, COLORS['white'])
        score_text = FONT.render(f"Pontos: {self.score}", True, COLORS['white'])
        level_text = FONT.render(f"Nível: {self.level}", True, COLORS['white'])
        lines_text = FONT.render(f"Linhas: {self.lines_cleared}", True, COLORS['white'])
        
        # Barra de progresso do nível
        level_progress = self.lines_cleared % 10
        pygame.draw.rect(self.game_surface, (80, 80, 100), (10, PANEL_HEIGHT-15, 100, 8))
        pygame.draw.rect(self.game_surface, COLORS['I'], (10, PANEL_HEIGHT-15, level_progress * 10, 8))
        
        self.game_surface.blit(time_text, (10, 10))
        self.game_surface.blit(score_text, (10, 30))
        self.game_surface.blit(level_text, (GAME_WIDTH//2 - level_text.get_width()//2, 10))
        self.game_surface.blit(lines_text, (GAME_WIDTH - lines_text.get_width() - 10, 10))
    ...
    def draw_board(self):
        ...
        board_rect = pygame.Rect(SIDEBAR_WIDTH, PANEL_HEIGHT, BOARD_WIDTH * BLOCK_SIZE, BOARD_HEIGHT * BLOCK_SIZE)
        pygame.draw.rect(self.game_surface, COLORS['panel'], board_rect)
        pygame.draw.rect(self.game_surface, COLORS['violeta'], board_rect, 3)
        
        # Efeito de linhas completadas
        if self.clear_effect and time.time() - self.clear_effect_time < 0.3:
            for y in self.clear_effect:
                highlight_rect = pygame.Rect(SIDEBAR_WIDTH, PANEL_HEIGHT + y * BLOCK_SIZE, 
                                        BOARD_WIDTH * BLOCK_SIZE, BLOCK_SIZE)
                highlight_surf = pygame.Surface((BOARD_WIDTH * BLOCK_SIZE, BLOCK_SIZE), pygame.SRCALPHA)
                highlight_surf.fill(COLORS['highlight'])
                self.game_surface.blit(highlight_surf, (SIDEBAR_WIDTH, PANEL_HEIGHT + y * BLOCK_SIZE))
        else:
            self.clear_effect = None
        ...
        # Grade do tabuleiro
        grid_surface = pygame.Surface((BOARD_WIDTH * BLOCK_SIZE, BOARD_HEIGHT * BLOCK_SIZE), pygame.SRCALPHA)
            ...
            for j, cell in enumerate(row):
                if cell:
                    ...
                    if abs(self.rotate_angle) > 0:
                        rotated_block = pygame.Surface((BLOCK_SIZE+10, BLOCK_SIZE+10), pygame.SRCALPHA)
                        block_rect = pygame.Rect(5, 5, BLOCK_SIZE, BLOCK_SIZE)
                        self.draw_block(rotated_block, 5, 5, self.current_shape, BLOCK_SIZE, True)
                        
                        # Aplica rotação
                        rotated_block = pygame.transform.rotate(rotated_block, self.rotate_angle)
                        self.rotate_angle *= 0.9  # Suaviza a rotação
                        
                        # Centraliza após rotação
                        block_rect = rotated_block.get_rect(center=(x + BLOCK_SIZE//2, y + BLOCK_SIZE//2))
                        self.game_surface.blit(rotated_block, block_rect)
                    else:
                        self.draw_block(self.game_surface, x, y, self.current_shape, BLOCK_SIZE, True)
        
        # DESENHA AS PARTÍCULAS AQUI (antes dos overlays)
        self.particles = [p for p in self.particles if p.update()]
        for p in self.particles:
            surf = pygame.Surface((p.size * 2, p.size * 2), pygame.SRCALPHA)
            pygame.draw.circle(surf, (*p.color, p.alpha), (p.size, p.size), p.size)
            self.game_surface.blit(surf, (int(p.x - p.size), int(p.y - p.size)))
        
        # Rodapé com controles
        footer_rect = pygame.Rect(0, PANEL_HEIGHT + BOARD_HEIGHT * BLOCK_SIZE, GAME_WIDTH, FOOTER_HEIGHT)
        ...
        for icon, text in controls:
            ...
            self.game_surface.blit(text_surf, (start_x + (icon_surf.get_width() - text_surf.get_width())//2, y_pos + 30))
        ...
        if self.paused:
            ...
            resume_text = FONT.render("Pressione P para continuar", True, COLORS['white'])
            
            self.game_surface.blit(pause_text, (GAME_WIDTH//2 - pause_text.get_width()//2, GAME_HEIGHT//2 - 50))
            self.game_surface.blit(resume_text, (GAME_WIDTH//2 - resume_text.get_width()//2, GAME_HEIGHT//2 + 10))
        ...
        if not self.running:
            ...
            score_text = LARGE_FONT.render(f"Pontuação: {self.score}", True, COLORS['white'])
            lines_text = LARGE_FONT.render(f"Linhas: {self.lines_cleared}", True, COLORS['white'])
            restart_text = FONT.render("Pressione R para reiniciar", True, COLORS['white'])
            
            self.game_surface.blit(game_over_text, (GAME_WIDTH//2 - game_over_text.get_width()//2, GAME_HEIGHT//2 - 80))
            self.game_surface.blit(score_text, (GAME_WIDTH//2 - score_text.get_width()//2, GAME_HEIGHT//2 - 20))
            self.game_surface.blit(lines_text, (GAME_WIDTH//2 - lines_text.get_width()//2, GAME_HEIGHT//2 + 20))
            self.game_surface.blit(restart_text, (GAME_WIDTH//2 - restart_text.get_width()//2, GAME_HEIGHT//2 + 80))
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                if self.game_state == "menu":
                    ...
                    if action == "start":
                        self.menu.start_transition("game")
                    elif action == "quit":
                        running = False
            ...
            if self.game_state == "menu":
                ...
                if result:  # Se a transição terminou
                    self.game_state = result
                    self.__init_game()  # Inicializa o jogo
    ...
    def __init_game(self):
        # Mantém as configurações de tela
        ...
        self.piece_x = 0
        self.piece_y = 0
        self.running = True  # Agora sim começa o jogo
        self.paused = False
        self.score = 0
        self.level = 1
        self.lines_cleared = 0
        ...
        # Restaura as configurações de tela
        self.fullscreen = fullscreen
        self.screen_width = screen_width
        self.screen_height = screen_height
        self.screen = pygame.display.set_mode(
            (self.screen_width, self.screen_height), pygame.FULLSCREEN if self.fullscreen else 0)
        
        # Inicia o jogo
        self.running = True
        self.spawn_piece()

        self.game_state = "game"  # Garante que o estado seja definido
        self.running = True  # Permite que o jogo comece
        self.spawn_piece()
